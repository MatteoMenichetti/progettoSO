...
int main(void) {
    ...
    while (0 == 0) {
        ...
        if ((read(fd[P1], vp + P1, sizeof(int))) == -1) { perror("DF: read P1"); }
        dprintf(voted_output, " p1: %d \n", vp + P1);
        if ((read(fd[P2], vp + P2, sizeof(int))) == -1) { perror("DF: read P2"); }
        dprintf(voted_output, " p2: %d \n", vp + P2);
        if ((read(fd[P3], vp + P3, sizeof(int))) == -1) { perror("DF: read P3"); }
